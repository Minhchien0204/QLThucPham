...
namespace DATN.Services.PhieuYeuCauService
{
    public class PhieuYeuCauService : IPhieuYeuCauService
    {
        ...
        public async Task DeletePYC(string sophieuyeucau)
        {
            ...
            var phieuyeucau = await _context.PhieuYeuCaus.Where(pyc => pyc.SoPhieuYeuCau == sophieuyeucau).Include(ctyc => ctyc.ChiTietYeuCaus).FirstOrDefaultAsync();
            if(phieuyeucau != null)
            {
                _context.PhieuYeuCaus.Remove(phieuyeucau);
                await _context.SaveChangesAsync();
            }
