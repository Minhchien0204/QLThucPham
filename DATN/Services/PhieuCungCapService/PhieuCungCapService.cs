...
namespace DATN.Services.PhieuCungCapService
{
    public class PhieuCungCapService : IPhieuCungCapService
    {
        ...
        {
            var phieucungcap = await _context.PhieuCungCaps.Where(pcc => pcc.SoPhieuCugCap == sophieucungcap).Include(ctcc => ctcc.ChiTietCungCaps).FirstOrDefaultAsync();
            if(phieucungcap != null)
            {
                _context.PhieuCungCaps.Remove(phieucungcap);
                await _context.SaveChangesAsync();
            }
