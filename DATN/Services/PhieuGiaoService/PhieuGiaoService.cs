...
namespace DATN.Services.PhieuGiaoService
{
    public class PhieuGiaoService : IPhieuGiaoService
    {
        ...
        {
            var phieugiao = await _context.PhieuGiaos.Where(pg => pg.SoPhieuGiao == sophieugiao).Include(ctg => ctg.ChiTietGiaos).FirstOrDefaultAsync();
            if(phieugiao != null)
            {
                _context.PhieuGiaos.Remove(phieugiao);
                await _context.SaveChangesAsync();
            }
