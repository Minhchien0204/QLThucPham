...
namespace DATN.Services.PhieuBanGiaoService
{
    public class PhieuBanGiaoService : IPhieuBanGiaoService
    {
        ...
        public async Task DeletePBG(string sophieubangiao)
        {
            ...
            var phieubangiao = await _context.PhieuBanGiaos.Where(pbg => pbg.SoPhieuBanGiao == sophieubangiao).Include(ctbg => ctbg.ChiTietBanGiaos).FirstOrDefaultAsync();
            if(phieubangiao != null)
            {
                _context.PhieuBanGiaos.Remove(phieubangiao);
                await _context.SaveChangesAsync();
            }
